...
# This code is to fix skvideo and numpy >= 1.20 compatibility issue
np.float = np.float64
np.int = np.int_

import cv2
from skvideo.io import vread
from skvideo.measure import viideo_score, strred, ssim
import torch
import yaml
...
def create_video_from_images(folder_path, video_name, dest_path, frames, fps=30) -> str:
    """
    Creates a video from all images in a given folder.

    Args:
        folder_path (str): Path to the folder containing the images.
        video_name (str): Name of the output video file (e.g., "output.avi").
        dest_path (str): Path to the output folder to save the video.
        frames (int): Number of frames to load.
        fps (int, optional): Frames per second for the output video. Default is 30.

    Returns:
        dest_video_path(str): Path to the output video.
    """
    ...
    # Sort the image files in alphabetical order (assuming they have sequential names)
    image_files.sort()
    image_files = image_files[:frames]
    ...
    dest_video_path = os.path.join(dest_path, video_name)
    video_writer = cv2.VideoWriter(dest_video_path, fourcc, fps, (width, height))
...
def calculate_strred(seq_ref_path: str, seq_den_path: str, frames: int):
    # open the reference sequence
    refseq = vread(seq_ref_path, num_frames=frames, as_grey=True)

    # open the denoised sequence
    denseq = vread(seq_den_path, num_frames=frames, as_grey=True)

    _, strred_score, _ = strred(refseq, denseq)

    return strred_score

def calculate_ssim(seq_ref_path: str, seq_den_path: str, frames: int):
    ...
    ssim_array = ssim(refseq, denseq)

    return ssim_array


def batch_psnr(img, imclean, data_range, enableLogging=False):
    r"""
    ...
    psnr = 0
    total_frames = img_cpu.shape[0]
    for i in range(total_frames):
        psnr_frame = peak_signal_noise_ratio(imgclean[i, :, :, :], img_cpu[i, :, :, :], data_range=data_range)
        psnr += psnr_frame
        if enableLogging:
            print(f'[Frame:{i}] PSNR: {psnr_frame:.2f}')

    return psnr/total_frames
...
def save_as_json(data, save_path):
    with open(save_path, 'w', encoding='utf-8') as f:
        json_str = json.dumps(data, default=lambda k: k.__dict__, indent=2)
        f.write(json_str)

def load_options(file_path) -> dict:
    with open(file_path, mode='r') as f:
        options_dict = yaml.load(f, Loader=yaml.FullLoader)
        return options_dict
...
def apply_padding(noisyframe, sigma_noise):
    ...
    padded_noisyframe = F.pad(input=noisyframe, pad=padexp, mode='reflect')
    padded_noisemap = F.pad(input=sigma_noise, pad=padexp, mode='reflect')

    return padded_noisyframe, padded_noisemap

def remove_padding(noisyframe, denoisedframe):
    expanded_h = denoisedframe.shape[-2] - noisyframe.shape[-2]
    expanded_w = denoisedframe.shape[-1] - noisyframe.shape[-1]
    if expanded_h:
        denoisedframe = denoisedframe[:, :, :-expanded_h, :]
    if expanded_w:
        denoisedframe = denoisedframe[:, :, :, :-expanded_w]
